...
K_bcc_path = '../015_BulkModulusDet/Results/bccFe.csv'
K_hcp_path = '../015_BulkModulusDet/Results/hcpFe_Murphy.csv'
pub_K_hcp_path = '../005_PubNRIXSVals/hcpFe_Murphy.csv'

start_fit_path = 'StartFitParams.csv'
Murphy_file_path = '../005_PubNRIXSVals/hcpFe_Murphy.csv'
...
def plot_AICc_analysis(psvl_all_df,binned_df,Gauss_opt,folder,index,fit_type,
	...
	if E_Murphy != None:
		Emin_Murphy = E_Murphy[0]
		Emax_Murphy = E_Murphy[1]
	...
	if E_Murphy != None:
		ax0.plot(Emax_Murphy,Emin_Murphy,'o',color='red')
	...
	if E_Murphy != None:
		ax1.plot(Emax_Murphy,Emin_Murphy,'o',color='red')
	...
	if E_Murphy != None:
		ax2.plot(Emax_Murphy,Emin_Murphy,'o',color='red')
...
pub_K_hcp_df = pd.read_csv(pub_K_hcp_path, engine='python')
...
for respath in respath_list:
	...
	# For hcp Fe, use reported K_S from Murphy et al. However, uncertainty isn't
	# reported, so use our calculated dK_S.
	# Remember to change this for hcp FeNi and FeNiSi!
	...
	if phase == 'hcp':
		P = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['P']
		dP = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['dP']
		V = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['V']
		dV = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['dV']
		rho = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['rho']
		drho = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['drho']
		KT = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['KT']
		dKT = K_hcp_df[K_hcp_df['Index']==index].iloc[-1]['dKT']
		KS = pub_K_hcp_df[pub_K_hcp_df['Index']==index].iloc[-1]['KS']
...
# # for folder in ['2009Oct_36GPa']: # For testing
# for folder in folder_list:
# 	index = index_dict[folder]
...
# Import Caitlin's choices for Emin and Emax. Not needed for FeNi and FeNiSi
# Delete for FeNi and FeNiSi
Murphy_df = pd.read_csv(Murphy_file_path)
...
E_Murphy = dict() # Not needed for FeNi and FeNiSi
E_choice = dict()

for folder in folder_list:
	index = index_dict[folder]

	# Not needed for FeNi and FeNiSi:
	if index in Murphy_df['Index'].values:
		Murphy_Emin = Murphy_df[Murphy_df['Index']==index].iloc[-1]['minE']
		Murphy_Emax = Murphy_df[Murphy_df['Index']==index].iloc[-1]['maxE']
		E_Murphy[folder] = [Murphy_Emin, Murphy_Emax]
	else:
		E_Murphy[folder] = None
...
for folder in folder_list:
	...
	temp_df = pd.DataFrame([[folder,index,vD,dvD,prob_amp,vphi,dvphi,vP,dvP,vS,dvS]],
	...
	plot_AICc_analysis(vel_results_df,binned_df,Gauss_opt,folder,index,fit_type,
		E_choice[folder],E_Murphy[folder],plot_title)
