...
INPUTFILE_RES="../../$FILE_NAME.res"
INPUTFILE_RFC="../../Output/$FILE_NAME\_rfc.dat"

OUTPUTFILE_RESPEAK="../ResPeak_$FILE_NAME.ps"
PDFFILE_RESPEAK="../ResPeak_$FILE_NAME.pdf"

BATCHFILE_RESPEAK="ResPeak.bfile"
...
#		the third word on the line with VAR1)
#	Then do a funny dance to properly save the new file
awk -v VAR1=\"$INPUTFILE_RFC\" '/READ XY/ {$3 = VAR1} {print $0}' $BATCHFILE_RESPEAK > input_file.tmp && mv input_file.tmp $BATCHFILE_RESPEAK
awk -v VAR1=\"$INPUTFILE_RES\" '/READ XYDY/ {$3 = VAR1} {print $0}' $BATCHFILE_RESPEAK > input_file.tmp && mv input_file.tmp $BATCHFILE_RESPEAK
awk -v VAR1=\"$OUTPUTFILE_RESPEAK\" '/PRINT TO/ {$3 = VAR1} {print $0}' $BATCHFILE_RESPEAK > input_file.tmp && mv input_file.tmp $BATCHFILE_RESPEAK

# Create the postscript file
xmgrace -batch ResPeak.bfile -nosafe -hardcopy

# Convert postscript file to pdf
pstopdf $OUTPUTFILE_RESPEAK

# Eliminate whitespace around pdf (pdfcrop input.pdf output.pdf, overrides file)
pdfcrop $PDFFILE_RESPEAK $PDFFILE_RESPEAK

# Delete postscript file to keep clutter down
rm $OUTPUTFILE_RESPEAK