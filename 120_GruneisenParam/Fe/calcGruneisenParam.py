...
cf0 = ax0.pcolormesh(q_array,gamma_array,np.log(pivotplot),edgecolor='face')
ax0.plot(1,2.04,'o',color='white')
ax0.plot(0.8,1.88,'^',color='white')
ax0.plot(1,1.98,'^',color='white')
ax0.plot(1.2,2.08,'^',color='white')
ax0.plot(0.78,1.74,'s',color='white')
cbar0 = plt.colorbar(cf0, ax = ax0)
cbar0.ax.set_ylabel(r'log($\chi^2$)',fontsize=16)
ax0.set_xlabel('$q$',fontsize=16)
ax0.set_ylabel('$\gamma_{vib,0}$',fontsize=16)
fig.savefig('Results/Fe_Gruneisen_misfit.pdf', format='pdf',
	bbox_inches='tight')
plt.close()
...
fig.savefig('Results/Fe_Gruneisen_pdf.pdf', format='pdf',
	bbox_inches='tight')
plt.close()
...
ax0.set_title('q=0.8')
fig.savefig('Results/Fe_misfit_q0.8.pdf', format='pdf',
	bbox_inches='tight')
plt.close()
...
# ax0.set_title('q=0.8')
# fig.savefig('Results/Fe_pdf_q0.8.pdf', format='pdf',
# 	bbox_inches='tight')
# plt.close()
...
print('q='+str(q)+':')
print(fit)
dgamma0_when_q08 = np.sqrt(np.diag(cov))
print(dgamma0_when_q08)
...
# ax0.set_ylim([350,600])
ax0.set_title('q=1.0')
fig.savefig('Results/Fe_misfit_q1.0.pdf', format='pdf',
	bbox_inches='tight')
plt.close()
...
# # ax0.set_ylim([0,10**(-160)])
# ax0.set_title('q=1.0')
# fig.savefig('Results/Fe_pdf_q1.0.pdf', format='pdf',
# 	bbox_inches='tight')
# plt.close()
...
print('q='+str(q)+':')
print(fit)
dgamma0_when_q1 = np.sqrt(np.diag(cov))
print(dgamma0_when_q1)
...
# ax0.set_ylim([350,600])
ax0.set_title('q=1.2')
fig.savefig('Results/Fe_misfit_q1.2.pdf', format='pdf',
	bbox_inches='tight')
plt.close()
...
# # ax0.set_ylim([0,10**(-160)])
# ax0.set_title('q=1.2')
# fig.savefig('Results/Fe_pdf_q1.2.pdf', format='pdf',
# 	bbox_inches='tight')
# plt.close()
...
print('q='+str(q)+':')
print(fit)
dgamma0_when_q12 = np.sqrt(np.diag(cov))
print(dgamma0_when_q12)
